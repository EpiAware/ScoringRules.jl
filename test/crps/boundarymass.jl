@testitem "BoundaryMass scores match R scoringRules" setup = [References] begin
    ...
    @testset "generalised truncated/censored Logistic" begin
        ...
        for i in 1:n
            d = BoundaryMass(
                Logistic(c["location"][i], c["scale"][i]);
                lower = c["lower"][i], upper = c["upper"][i],
                lmass = c["lmass"][i], umass = c["umass"][i]
            )
            @test crps(d, c["y"][i]) ≈ c["crps"][i] atol = atol rtol = rtol
...
@testitem "BoundaryMass consistency and interface" begin
    ...
    @testset "zero masses reduce to truncation" begin
        ...
        for (base, l, u, rt) in [
                (Normal(0.5, 1.2), -1.0, 2.0, 1.0e-12),
                (Logistic(0.0, 0.8), -1.5, 1.5, 1.0e-12),
                (TDist(5), -1.0, 2.0, 1.0e-12),
                (Uniform(-1.0, 2.0), -0.5, 1.5, 1.0e-8),
                (Exponential(2.0), 0.5, Inf, 1.0e-8),
            ]
            w = BoundaryMass(base; lower = l, upper = u)
            t = truncated(base; lower = l, upper = isfinite(u) ? u : nothing)
            for y in ys
                @test crps(w, y) ≈ crps(t, y) rtol = rt atol = 1.0e-9
            end
        end
    end
    ...
    @testset "logistic closed form stays finite for far-out bounds" begin
        ...
        w = BoundaryMass(
            Logistic(0, 1); lower = -5.0, upper = 100.0,
            lmass = 0.1, umass = 0.1
        )
    ...
    @testset "distribution interface" begin
        ...
        @test maximum(w) == 2
        @test cdf(w, -1.5) == 0
        @test cdf(w, -1) ≈ 0.1 atol = 1.0e-12
        @test cdf(w, 2) == 1
        @test cdf(w, 0.5) ≈ 0.1 + 0.7 * cdf(int, 0.5) atol = 1.0e-12
        @test pdf(w, -1) == 0.1        # atom reported as its mass (as Censored)
        @test pdf(w, 2) == 0.2
        @test pdf(w, 0.5) ≈ 0.7 * pdf(int, 0.5) atol = 1.0e-12
        @test logpdf(w, -1) ≈ log(0.1) atol = 1.0e-12
        @test logpdf(w, 3) == -Inf
        @test quantile(w, 0.05) == -1
        @test quantile(w, 0.95) == 2
        @test quantile(w, 0.5) ≈ quantile(int, (0.5 - 0.1) / 0.7) atol = 1.0e-12
        @test mean(w) ≈ -1 * 0.1 + 2 * 0.2 + 0.7 * mean(int) atol = 1.0e-12
        m = mean(w)
        ex2 = 0.1 * 1 + 0.2 * 4 + 0.7 * (var(int) + mean(int)^2)
        @test var(w) ≈ ex2 - m^2 atol = 1.0e-12
    ...
    @testset "constructor validation" begin
        ...
        @test_throws DomainError BoundaryMass(
            Normal(); lower = 0, upper = 1,
            lmass = 0.6, umass = 0.6
        )
    end
    ...
    @testset "unrouted combinations fall back to quadrature" begin
        ...
        wg = BoundaryMass(
            Gamma(2.0, 1.0); lower = 0.5, upper = 3.0,
