...
matching R's `crps_sample` and `dss_sample`.

`logs(dat, y; w)` also accepts member weights, which R's `logs_sample` does
not. The weighted score evaluates the kernel density
